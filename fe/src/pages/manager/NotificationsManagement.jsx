...
const NotificationsManagement = () => {
    ...
    const mockNotifications = [
      {
        ...
        message: 'Bệnh viện đang thiếu máu O- nghiêm trọng. Cần người hiến máu khẩn cấp.',
      ...
      },
      {
        ...
        message: 'Thông báo lịch hiến máu định kỳ tháng 12/2024. Đăng ký tại website hoặc hotline.',
      ...
      },
      {
        ...
        message: 'Đã đến thời gian khám sức khỏe định kỳ cho người hiến máu thường xuyên.',
      ...
      },
      {
        ...
        message: 'Thông báo về quy trình hiến máu mới có hiệu lực từ ngày 15/12/2024.',
  ...
  useEffect(() => {
    ...
    if (filters.recipient !== 'all') {
      filtered = filtered.filter(notif => notif.recipients === filters.recipient);
    }
  ...
  const getTypeText = (type) => {
    switch (type) {
      case NOTIFICATION_TYPES.INFO: return 'Thông tin';
      case NOTIFICATION_TYPES.WARNING: return 'Cảnh báo';
      case NOTIFICATION_TYPES.EMERGENCY: return 'Khẩn cấp';
      case NOTIFICATION_TYPES.REMINDER: return 'Nhắc nhở';
      case NOTIFICATION_TYPES.ANNOUNCEMENT: return 'Thông báo';
      default: return 'Khác';
    }
  };
  ...
  const getStatusText = (status) => {
    switch (status) {
      case NOTIFICATION_STATUS.PENDING: return 'Chờ gửi';
      case NOTIFICATION_STATUS.SENT: return 'Đã gửi';
      case NOTIFICATION_STATUS.SCHEDULED: return 'Đã lên lịch';
      case NOTIFICATION_STATUS.FAILED: return 'Thất bại';
  ...
  const getRecipientText = (recipients) => {
    switch (recipients) {
      ...
      case 'donors': return 'Người hiến máu';
      case 'recipients': return 'Người cần máu';
  ...
  const getTypeColor = (type) => {
    switch (type) {
      case NOTIFICATION_TYPES.INFO: return 'info';
      case NOTIFICATION_TYPES.WARNING: return 'warning';
      case NOTIFICATION_TYPES.EMERGENCY: return 'danger';
      case NOTIFICATION_TYPES.REMINDER: return 'primary';
      case NOTIFICATION_TYPES.ANNOUNCEMENT: return 'success';
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case NOTIFICATION_STATUS.PENDING: return 'warning';
      case NOTIFICATION_STATUS.SENT: return 'success';
      case NOTIFICATION_STATUS.SCHEDULED: return 'info';
      case NOTIFICATION_STATUS.FAILED: return 'danger';
      default: return 'secondary';
    }
  };
  ...
  const handleCreateNotification = () => {
    const notification = {
      ...
      status: newNotification.isScheduled ? NOTIFICATION_STATUS.SCHEDULED : NOTIFICATION_STATUS.PENDING,
      urgency: newNotification.urgency,
      createdAt: new Date().toISOString(),
      scheduledTime: newNotification.isScheduled ? newNotification.scheduledTime : null,
      readCount: 0,
      responseCount: 0
    };
  ...
  const getRecipientCount = (recipients) => {
    switch (recipients) {
      case 'all': return 200;
      case 'donors': return 120;
      case 'recipients': return 55;
      ...
      <div className="notifications-content">
        <div className="page-header">
          <div>
            ...
            <p>Tạo và gửi thông báo đến người dùng hệ thống</p>
          </div>
          <button 
        ...
        <div className="notifications-list">
          {filteredNotifications.map(notification => (
            <div key={notification.id} className={`notification-card ${getTypeColor(notification.type)}`}>
                ...
                <div className="notification-info">
                  ...
                  <div className="notification-meta">
                    ...
                    <span className="recipient-info">
                      👥 {getRecipientText(notification.recipients)} ({notification.recipientCount})
                    </span>
                  </div>
                </div>
                ...
                <div className="notification-stats">
                  ...
                  {notification.scheduledTime && (
                    <div className="stat-item">
                      ...
                      <span className="stat-value">
                        {new Date(notification.scheduledTime).toLocaleString('vi-VN')}
                      </span>
                    </div>
                  )}
                  ...
                  {notification.status === NOTIFICATION_STATUS.SENT && (
                    <>
                      ...
                      <div className="stat-item">
                        ...
                        <span className="stat-value">{notification.responseCount}</span>
                      </div>
                    </>
                  )}
                </div>
        ...
        <div className="notification-stats-summary">
          ...
          <div className="stat-card">
            ...
            <p className="stat-number warning">
              {notifications.filter(n => n.status === NOTIFICATION_STATUS.PENDING).length}
            </p>
          </div>
          <div className="stat-card">
            <h3>Đã gửi</h3>
            <p className="stat-number success">
              {notifications.filter(n => n.status === NOTIFICATION_STATUS.SENT).length}
            </p>
          </div>
          <div className="stat-card">
            <h3>Khẩn cấp</h3>
            <p className="stat-number danger">
              {notifications.filter(n => n.type === NOTIFICATION_TYPES.EMERGENCY).length}
            </p>
          </div>
        </div>
      </div>
              ...
              <div className="form-group">
                <label className="checkbox-label">
                  <input 
                    ...
                    onChange={(e) => setNewNotification(prev => ({...prev, isScheduled: e.target.checked}))}
                  />
                  Lên lịch gửi
                </label>
              </div>
